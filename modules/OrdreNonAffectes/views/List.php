...
class OrdreNonAffectes_List_View extends Vtiger_List_View
{
	...
	public function process(\App\Request $request)
	{
		...
		$viewer->view('ListVieworders.tpl', $moduleName);
	...
	public function initializeListViewContents(\App\Request $request, Vtiger_Viewer $viewer)
	{
		...
		if (!$this->listViewEntries) {
			    ...
			    $this->listViewEntries=$mysqli->query("SELECT cmd.ssingleordersid,cmd.num_ordre,cmd.num_bc,cmd.support,cmd.type_ordre,cmd.date_ordre,cmd.edition,cmd.raison_sociale,cmd.nom_commercial,cmd.nom_signataire
			                FROM u_yf_ssingleorders cmd
			                INNER JOIN vtiger_crmentity c on cmd.ssingleordersid = c.crmid 
			                where (select count(*) from u_yf_ssingleorders_inventory inv where inv.id=cmd.ssingleordersid)> (select count(*) from u_yf_atribution attr where attr.ordre=cmd.ssingleordersid)")->fetch_all();
