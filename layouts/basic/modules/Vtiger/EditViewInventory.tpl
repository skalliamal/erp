							...
							{if \App\Module::isModuleActive($MAIN_MODULE)}
								...
								<span class="btn-group-sm d-flex align-items-center justify-content-center ml-lg-2">
										<button type="button" data-module="{$MAIN_MODULE}"
											...
											<span class="fas fa-plus"></span> &nbsp;<strong>Ajouter</strong>{*<strong>{\App\Language::translate('LBL_ADD',$MODULE)} {\App\Language::translate('SINGLE_'|cat:$MAIN_MODULE,$MAIN_MODULE)}</strong> *}
										</button>
									</span>
							{/if}
						{/foreach}
