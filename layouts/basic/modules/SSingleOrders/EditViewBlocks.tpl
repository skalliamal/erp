...
function handleBCValidation(){
	//ajouter ajax call pour vérifier Client débiteur Ed. antérieure
	var docIsChecked = $('#SSingleOrders_editView_fieldName_docs_pub_recus').prop('checked');
	var debitIsChecked = $('#SSingleOrders_editView_fieldName_client_debiteur_ed_anterieur').prop('checked');
	$('#SSingleOrders_editView_fieldName_derogation').prop('disabled', !debitIsChecked);
	var derogIsChecked = $('#SSingleOrders_editView_fieldName_derogation').prop('checked');
	$('#SSingleOrders_editView_fieldName_validation_bc').prop('disabled', (!docIsChecked || debitIsChecked) && (!docIsChecked || !debitIsChecked || !derogIsChecked));
	$('#SSingleOrders_editView_fieldName_docs_pub_recus, #SSingleOrders_editView_fieldName_client_debiteur_ed_anterieur, #SSingleOrders_editView_fieldName_derogation').change(function(){
		docIsChecked = $('#SSingleOrders_editView_fieldName_docs_pub_recus').prop('checked');
		debitIsChecked = $('#SSingleOrders_editView_fieldName_client_debiteur_ed_anterieur').prop('checked');
		derogIsChecked = $('#SSingleOrders_editView_fieldName_derogation').prop('checked');
		$('#SSingleOrders_editView_fieldName_validation_bc').prop('disabled', (!docIsChecked || debitIsChecked) && (!docIsChecked || !debitIsChecked || !derogIsChecked));
		$('#SSingleOrders_editView_fieldName_validation_bc').prop('checked', (docIsChecked && !debitIsChecked) || (docIsChecked && debitIsChecked && derogIsChecked));
		$('#SSingleOrders_editView_fieldName_derogation').prop('disabled', !debitIsChecked);
		if(!debitIsChecked)
			$('#SSingleOrders_editView_fieldName_derogation').prop('checked', false);
	});
}
});
</script>