...
$(document).ready(function() {
	...
	let url = "index.php?action=Edicom&method=getInvoiceAllowedAmount&record=" + record + "&relatedModule=" + relatedModule;
	AppConnector.request(url).done(function (data) {
		grossAmount = Number(data.result.grossAmount);
		sum = Number(data.result.sum);
		allowedAmount = grossAmount - sum;
	});
	$(document).off('change', '#Reglements_editView_fieldName_montant');
	$(document).off('change', '#Avoirs_editView_fieldName_montant_ttc');
	$(document).off('shown.bs.modal', '.modal');

	$(document).on('shown.bs.modal', '.modal', function(){
		console.log(sum);
		$('#Reglements_editView_fieldName_reg_recu').val(sum);
		$('#Reglements_editView_fieldName_reg_a_recevoir').val(allowedAmount);
	});
	$(document).on('change', '#Reglements_editView_fieldName_montant', function(){
	...
	$(document).on('change', '#Avoirs_editView_fieldName_montant_ttc', function(){
		if($(this).val() > allowedAmount){
			Vtiger_Helper_Js.showPnotify("Montant supérieur au montant permis " + allowedAmount);
			$(this).val(null);
		}
	...
	$(document).on('change', '#Avoirs_editView_fieldName_taux', function(){
