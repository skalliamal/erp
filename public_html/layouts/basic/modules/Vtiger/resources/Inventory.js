...
$.Class("Vtiger_Inventory_Js", {}, {
	...
	registerClearReferenceSelection: function (container) {
		...
		container.on('click', '.clearReferenceSelection', function (e) {
			var element = $(e.currentTarget);
			if(element.closest('td').find('input[title=Rubriques]').val() == 'Rubriques')
				return;
			var row = thisInstance.getClosestRow(element);
	...
	registerRowAutoComplete: function (container) {
		...
		sourceFieldElement.on(Vtiger_Edit_Js.referenceSelectionEvent, function (e, params) {
			...
			AppConnector.request(dataUrl).done(function (data) {
				if(element.data('columnname') == 'name'){
					edicomInventoryHandler(record, element);
				}
				if(element.data('columnname') == 'ref'){
					return;
				}
				for (var id in data) {
...
function edicomInventoryHandler(record, element){
	let url = "index.php?action=Edicom&record=" + record + "&method=getProductInfo";
	AppConnector.request(url).done(function (data) {
		element.closest('tr').find('[name^=int1]').val(data.result.duree_affichage);
		element.closest('tr').find('[name^=picklist1]').val(data.result.unite_affichage).trigger('change.select2');
		element.closest('tr').find('[name^=int20]').val(data.result.nombre_affichage);
	}).fail(function (error) {
		console.log(error);
	});
}